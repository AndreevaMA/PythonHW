...
my_list = [round(random.random() * 100, 2) for i in range(random.randint(5, 10))]
print(my_list)

# используем filter
list_no_int = list(filter(lambda x: x % 1 != 0, my_list)) # в указанном примере число 5 считается числом без дробной части
print(list_no_int)
